...
| **PO-DEC-10** | **Locum billing rule** (PUB-064: part-time locums free, full-time at staff rate) | **External commercial policy** consuming a versioned read-only membership/FTE projection; **no product billing capability** | Billing must be derived from, never embedded in, scheduling data | product-scope billing capability | scheduling, opportunities, stipends, and entitlements silently become an invoicing system | CAP-005, CAP-013, CAP-057 (projection only) | low | medium | no | yes | **pending — restored 2026-08-01** |
...
> **Register repair, 2026-08-01 (independent-review finding CAR-016).** **`PO-DEC-10` — Locum billing rule** was created in [report 17](17-public-source-gap-addendum.md) §11 and was **omitted from this register without a supersession record**, so its absence could not safely be read as a decision. **By explicit product-owner decision it is restored above with its original identifier and a status of `pending`.** No identifier has been renumbered or reused, and no historical record has been rewritten. **The decision itself remains unratified**, and its working default — locum billing as external commercial policy consuming a read-only projection — may be used for planning only. **Pending decisions now number 19.**
