...
| **T-08** *(mitigation amended 2026-08-01, V-20)* | **Real-time subscription leakage** | Subscribes to another tenant's picklist | **Per-frame declared context + per-frame authorization** — every command frame re-declares the tenant and re-runs the full truth table against current state. The previous mitigation, a context bound once at connection establishment, is **withdrawn**: it leaves a revoked privilege effective for the life of the socket, which is the CAR-008 defect ([SPEC-01](specs/SPEC-01-request-context-and-tenant-isolation.md) §5, [SPEC-06](specs/SPEC-06-authorization-truth-table.md) §6) | Per-topic authorization at subscribe **and re-evaluation on every push** for sensitive classes (SPEC-06 §6.1); denials logged as security events; affected subscriptions closed immediately on revocation | SBX-023 |
