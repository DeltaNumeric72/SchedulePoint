...
> **AMENDED 2026-08-01 — measured, not assumed (EV-M0-SPC H-6/H-7, executed spike).** A fixed seed plus a fixed worker count is **NOT sufficient** for reproduction: CP-SAT's default parallel portfolio synchronises on thread timing (5 distinct schedules in 5 runs at 8 workers, same seed, same objective), and a wall-clock deadline makes the stopping point machine-dependent. **Reproducible builds therefore require, and the reproducibility record must pin, all of:** solver version/image digest · seed · worker count · **`interleave_search = true`** (the deterministic portfolio) · **deterministic time limits (`max_deterministic_time`), never wall-clock, on any reproducibility-claiming build** · the full remaining parameter set. Cost of the deterministic portfolio is instance-dependent (measured ~12× slower on a toy instance, *faster* on a hard one) and is measured on the E1 corpus before defaults are chosen. Explanation tier T1 uses CP-SAT assumptions (verified working: a 3-literal minimal core) but **as a separate, budgeted, failable solve** — blanket reification measurably destroys presolve (18.6 ms INFEASIBLE → 90 s UNKNOWN). Cancellation: watcher-thread `StopSearch()` (17 ms measured) preferred over solution callbacks (113 ms); **POSIX signals never** (handler starves while the GIL-holding thread is in native code; 28 s latency, misreported outcome). Production solver image pins **Python 3.12** (3.9 is EOL); the spike harness re-runs under 3.12 in CI before E0 closes.
