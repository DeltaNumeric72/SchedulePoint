...
--
-- ═══════════════════════════════════════════════════════════════════════════
-- 5. §7's CHECK on `vacation_selections` — the other half FAD-59 names
-- ═══════════════════════════════════════════════════════════════════════════
--
...
--
-- ═══════════════════════════════════════════════════════════════════════════
-- 7. And the two other things this migration adds
-- ═══════════════════════════════════════════════════════════════════════════
--
...
-- was never visible to be projected. Section 5 of the up migration is migration
-- 0012's enforcement-read mechanism applied to that, with 0012's three
-- confinement properties unchanged and its reasoning quoted where it differs.
-- It is read there rather than summarised here, because a widened read plane is
-- the last thing in this file a reader should meet as a footnote.
--
-- And §5 adds `vacation_commit` to the assignment ORIGIN vocabulary, because
-- §5.6's OFF snapshots are placed by a mechanism migration 0009's four-value
-- domain does not name, and a snapshot's origin is frozen the moment its version
-- publishes. That section carries the measurement the decision rests on.
...
------------------------------------------------------------------------------
-- 6. The SOLVER-PROJECTION read plane — migration 0012's mechanism, six tables
...
-- these six tables and `qualification_requirements` opens
