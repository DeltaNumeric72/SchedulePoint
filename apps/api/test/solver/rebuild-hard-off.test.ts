...
/**
 * **One date per CASE, and that is an order-independence rule rather than a
 * naming convention.**
 *
 * The composed run shuffles TESTS as well as files
 * (`--sequence.shuffle.tests`), so a case that relied on a request an earlier
 * case created fails whenever the shuffle puts it first — which is exactly what
 * happened to this file at seed 20260831, and it is the same defect class FU-32
 * records against `vacation-selection-http.test.ts`. The cure is the one FU-32
 * names and R-13/T-15 already use: **own-fixture-per-case**. Each case creates
 * the requests it is about and asserts ONLY about its own dates, so no ordering
 * of the three can change any verdict.
 */
const OFF_DATE_REBUILD = '2027-06-08';
const OFF_DATE_ELIGIBLE = '2027-06-09';
const WITHDRAWN_DATE = '2027-06-10';
const OFF_DATE_PLANE = '2027-06-11';
...
/**
 * A `time-off` request for `targetDate`, walked to `status`.
 *
 ...
 */
async function timeOffAt(status: string, targetDate: string): Promise<string> {
...
describe('SPEC-08 R-14 — the rebuild honours what a published version promised', () => {
  it('a `reflected_in_version` time-off request is still HardOff on the REBUILD', async () => {
    ...
    /* Scoped to THIS case's date, not to the membership: another case may have
     * created a row for the same person on ITS date, and a control that
     * demanded an empty list would then fail on an ordering rather than on a
     * defect. What must be true is that nothing yet projects THIS date. */
    expect(
      hardOffDatesFor(before.document, absentMembershipId),
      'the control must not already carry this case\'s date',
    ).not.toContain(OFF_DATE_REBUILD);

    /* The request, walked to the status V-31 is about: a PUBLISHED version
     * already honours this day off. */
    await timeOffAt('reflected_in_version', OFF_DATE_REBUILD);
  ...
  it('only ELIGIBLE rows appear — a withdrawn request is not projected', async () => {
     ...
     * rule module. */
    /* Own-fixture: this case creates the honoured request it asserts is present
     * as well as the withdrawn one it asserts is absent, so it holds whatever
     * order the shuffle puts it in. */
    await timeOffAt('reflected_in_version', OFF_DATE_ELIGIBLE);
    ...
    /* This case's OWN honoured row IS there — the non-vacuity control, so the
     * absence below is an absence rather than an empty projection — and its own
     * withdrawn day is NOT. */
    const dates = hardOffDatesFor(document.document, absentMembershipId);
    expect(dates, 'the honoured day is projected').toContain(OFF_DATE_ELIGIBLE);
    expect(dates, 'a withdrawn request never enters the projection').not.toContain(
      WITHDRAWN_DATE,
    );
  }, 240_000);
  ...
  it('the projection is assembled on the BUILDER\'s transaction and needs no read capability', async () => {
    ...
    /* Own-fixture again: the last assertion is that the projection is
     * POPULATED, which is only true if this case put something in it. */
    await timeOffAt('reflected_in_version', OFF_DATE_PLANE);
    ...
    expect(
      hardOffDatesFor(document.document, absentMembershipId),
      'the plane must have produced this case\'s own row',
    ).toContain(OFF_DATE_PLANE);
  }, 240_000);
});
