...
/**
 * A PUBLISHED version inside a round's OWN schedule period.
 *
 * In the round's own period rather than a shared one, so §5.6's draft-only rule
 * is refused BY NAME (`COMMIT_TARGET_NOT_DRAFT`) rather than by the period
 * mismatch, which would be the right answer for the wrong reason.
 */
async function publishedVersionFor(roundId: string): Promise<string> {
  const alpha = multi().alpha;
  const schedulePeriodId = SCHEDULE_PERIOD_BY_ROUND.get(roundId);
  if (schedulePeriodId === undefined) throw new Error('the fixture round has no schedule period');
  const versionId = randomUUID();
  await asScheduler(
    async ({ query }) =>
      await sql`
        insert into schedule_versions
          (id, organization_id, group_id, period_id, state, version_number, published_at,
           published_by)
        values (${versionId}::uuid, ${alpha.organizationId}::uuid, ${alpha.groupOne.id}::uuid,
                ${schedulePeriodId}::uuid, ${'published'}, 1, now(),
                ${alpha.users.scheduler.membershipId}::uuid)
      `.execute(query),
  );
  return versionId;
}
...
/**
 * **A quota round of this case's OWN, with its entitlement.**
 *
 * ONE ROUND PER CASE, and it is an order-independence rule rather than tidiness.
 * A commit is an act on the ROUND — it takes EVERY approved selection in the
 * period — so two cases sharing a round make each one's committed-selection
 * count depend on which ran first. `--sequence.shuffle.tests` shuffles cases, so
 * that dependence is a real failure and not a hypothetical: it fired at seed
 * 20260901 as `expected [ …(2) ] to have a length of 1 but got 2`. This is the
 * same defect class FU-32 records against `vacation-selection-http.test.ts`, and
 * the cure is the one FU-32 names — own-fixture-per-case, the R-13/T-15 pattern.
 */
async function ownQuotaRound(startDate: string, units = 4): Promise<string> {
  const roundId = await createPeriod(startDate, 'quota');
  await createEntitlement(roundId, units);
  return roundId;
}
...
describe('SPEC-08 §5.6 — the commit', () => {
  it('R-12 — a replayed key commits ONCE and the replay writes nothing', async () => {
    const round = await ownQuotaRound('2090-06-01');
    await approvedWeek(round, weekOf(round, 0));
    ...
    const first = await call(
      ...
      multi().alpha.users.scheduler.id,
      { targetVersionId: draftFor(round), idempotencyKey },
    );
    ...
    const afterFirst = await offSnapshots(draftFor(round));
    ...
    expect(await offSnapshots(draftFor(round))).toEqual(afterFirst);
  ...
  it('the selection and its root move together — §5.3, one transaction', async () => {
    const round = await ownQuotaRound('2091-06-01');
    ...
    const committed = await call(
      ...
      `${scope()}/vacation/rounds/${round}/commit`,
      multi().alpha.users.scheduler.id,
      { targetVersionId: draftFor(round), idempotencyKey: key('vc.pair') },
    );
    expect(committed.statusCode, committed.raw).toBe(200);

    const after = await selectionOf(requestId);
    expect(after.status).toBe('committed');
    expect(after.committed_to_version_id).toBe(draftFor(round));
  ...
  it('committing into a PUBLISHED version is refused BY NAME — I-18', async () => {
    const round = await ownQuotaRound('2092-06-01');
    await approvedWeek(round, weekOf(round, 2));
    const publishedVersionId = await publishedVersionFor(round);
  ...
  it('I-02 — `vacation.commit` is grant-only; without it the scheduler is refused', async () => {
    ...
    const round = await ownQuotaRound('2093-06-01');
    await approvedWeek(round, weekOf(round, 3));
    ...
    const denied = await call(
      ...
      `${scope()}/vacation/rounds/${round}/commit`,
      multi().alpha.users.scheduler.id,
      { targetVersionId: draftFor(round), idempotencyKey: key('vc.deny') },
    );
    ...
    const allowed = await call(
      ...
      `${scope()}/vacation/rounds/${round}/commit`,
      multi().alpha.users.scheduler.id,
      { targetVersionId: draftFor(round), idempotencyKey: key('vc.allow') },
    );
