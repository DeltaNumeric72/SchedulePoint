...
describe('migration 0027 over a populated database', () => {
  ...
  it('FAD-59\'s per-selection CHECK refuses BOTH directions', async () => {
    ...
    await expect(
      ...
      code: '23514',
      constraint: 'vacation_selections_committed_version_coherent',
    });
     ...
     * a row that CLAIMS to be committed and names no version.
     *
     * ── THE PROBE ROW CARRIES A REAL `request_id`, and that is the whole point ─
     *
     * The first version of this probe left `request_id` NULL, and review
     * falsified it: with `status = 'committed'` and no request, 0022's
     * `vacation_selections_request_present_unless_available` is violated TOO, so
     * the row was refused by two constraints at once and the case passed whether
     * or not FAD-59's existed. Which one Postgres reports is its evaluation
     * order, not a property of this schema, so the assertion rested on something
     * nobody had promised.
     *
     * The root below makes 0022's equality TRUE — `status <> 'available'` and
     * `request_id IS NOT NULL`, so `false = false` — leaving FAD-59's constraint
     * as the ONLY one this row violates. **And the constraint is asserted BY
     * NAME**, so even a future row that violated two would fail here rather than
     * pass on whichever fired first.
     *
     * Both statements run in ONE unit of work: the INSERT aborts the
     * transaction, so the root never commits and D-18's deferred zero-row guard —
     * which would legitimately object to a vacation root with no selection — is
     * never reached. */
    await expect(
      asMember(async ({ query }) => {
        const rootId = randomUUID();
        await sql`
          insert into requests
            (id, organization_id, group_id, membership_id, subtype, status, expires_at,
             idempotency_key)
          values (${rootId}::uuid, ${member.organizationId}::uuid, ${member.groupId}::uuid,
                  ${member.membershipId}::uuid, ${'vacation-selection'},
                  app_request_initial_status(${'vacation-selection'}),
                  ${'2099-06-01T00:00:00.000Z'}::timestamptz,
                  ${`cycle27.coherent.${randomUUID().slice(0, 12)}`})
        `.execute(query);
        await sql`
          insert into vacation_selections
          ...
          values (${member.organizationId}::uuid, ${member.groupId}::uuid,
                  ${member.membershipId}::uuid, ${vacationPeriodId}::uuid,
                  ${'2071-06-08'}::date, ${'committed'}, ${rootId}::uuid, ${null})
        `.execute(query);
      }),
...
describe('migration 0027 §6 — the solver-projection read plane', () => {
  ...
  it('the seven arms are BYTE-IDENTICAL to each other — sibling identity, not a substring sweep', async () => {
     ...
     * boundary.
     *
     * ── WHY IDENTITY, AND NOT "the qual CONTAINS these strings" ──────────────
     *
     * The first version of this case asserted that each qual CONTAINED
     * `current_setting('app.organization_id'…)`, `current_setting('app.group_id'…)`
     * and the token. Review falsified it: dropping the `group_id = …` EQUALITY
     * from one arm while keeping the `IS NOT NULL` guard leaves both substrings
     * present, so the case passed — and the full 205-file gate caught nothing
     * either, because every behavioural case on this plane is single-group and
     * the tenancy sweep never opens the token. A substring assertion cannot tell
     * a predicate from a mention of the same setting.
     *
     * Identity can. The seven arms are written to be the same predicate, so the
     * strongest available statement is that they ARE the same predicate — and
     * a mutation to any ONE of them makes the set disagree.
     *
     * **Measured fact behind the normaliser:** no table name appears in any of
     * these quals, because the predicate names only COLUMNS
     * (`organization_id`, `group_id`) and Postgres renders them unqualified. The
     * table name is normalised out anyway, so an arm that ever needed to qualify
     * a column stays comparable instead of failing for the wrong reason.
     *
     * Identity alone would still admit a WHOLESALE drop — all seven loosened
     * together — so the shared predicate is then asserted clause by clause, and
     * the `group_id` clause is asserted as the EQUALITY rather than by naming
     * the setting. Those two assertions catch opposite mutations, which is why
     * both are here. */
    const rows = await asScheduler(
      async ({ query }) =>
    ...
    const normalised = rows.rows.map((row) => ({
      tablename: row.tablename,
      qual: row.qual.split(row.tablename).join('<table>'),
    }));
    const distinct = [...new Set(normalised.map((row) => row.qual))];
    expect(
      distinct,
      `the seven projection arms do not share one predicate:\n${normalised
        .map((row) => `  ${row.tablename}: ${row.qual}`)
        .join('\n')}`,
    ).toHaveLength(1);

    /* The shared predicate, clause by clause. Each is quoted as Postgres renders
     * it, so a clause that changed SHAPE — not merely one that vanished — fails
     * here too. */
    const shared = distinct[0] ?? '';
    expect(shared, 'the organization predicate is an EQUALITY').toContain(
      "(organization_id = (NULLIF(current_setting('app.organization_id'::text, true), ''::text))::uuid)",
    );
    expect(shared, 'the group predicate is an EQUALITY, not merely a mention of the setting').toContain(
      "(group_id = (NULLIF(current_setting('app.group_id'::text, true), ''::text))::uuid)",
    );
    expect(shared, "the A-03b nullif guard is present").toContain(
      "(NULLIF(current_setting('app.group_id'::text, true), ''::text) IS NOT NULL)",
    );
    expect(shared, 'the arm is gated on the purpose token').toContain(
      "(NULLIF(current_setting('app.solver_projection_read'::text, true), ''::text) = 'solver_projection'::text)",
    );
  });

  it('the open plane does NOT cross a GROUP boundary — behavioural, under the token', async () => {
    /* The behavioural companion to the identity case above, and the one that
     * would have failed under review's mutation for a reason a reader can act
     * on: it asks what the plane SHOWS rather than what the policy SAYS.
     *
     * Every other behavioural case on this plane is single-group, which is
     * exactly why the dropped `group_id` equality survived the whole gate. This
     * one puts a real selection in Alpha's GROUP TWO and reads from GROUP ONE
     * with the token open.
     *
     * The control comes first and is not optional: the same read WITHOUT the
     * token must also not see it, so "invisible" cannot be an accident of the
     * row failing to exist. */
    const alpha = multi().alpha;
    const groupTwo = {
      organizationId: alpha.organizationId,
      groupId: alpha.groupTwo.id,
      membershipId: alpha.users.groupTwoScheduler.membershipId,
      correlationId: 'cycle27-group-two',
    };

    const foreignSelectionId = await runtime.runner.run(groupTwo, async ({ query }) => {
      const period = await sql<{ id: string }>`
        insert into vacation_periods
          (organization_id, group_id, start_date, end_date, mode, state)
        values (${groupTwo.organizationId}::uuid, ${groupTwo.groupId}::uuid,
                ${'2072-06-06'}::date, ${'2072-07-08'}::date, ${'quota'}, ${'open'})
        returning id
      `.execute(query);
      ...
      const selection = await sql<{ id: string }>`
        insert into vacation_selections
          (organization_id, group_id, membership_id, vacation_period_id, week_start, status)
        values (${groupTwo.organizationId}::uuid, ${groupTwo.groupId}::uuid,
                ${groupTwo.membershipId}::uuid, ${periodId}::uuid, ${'2072-06-06'}::date,
                ${'available'})
        returning id
      `.execute(query);
      return selection.rows[0]?.id ?? '';
    });
    expect(foreignSelectionId, 'the foreign fixture row must exist').not.toBe('');

    /* Non-vacuity: it IS visible in its own group, so the two readings below are
     * about the boundary rather than about a row nobody can see. */
    const inItsOwnGroup = await runtime.runner.run(
      groupTwo,
      async ({ query }) =>
        await sql<{ n: string }>`
          select count(*)::text as n from vacation_selections where id = ${foreignSelectionId}::uuid
        `.execute(query),
    );
    expect(Number(inItsOwnGroup.rows[0]?.n ?? '0')).toBe(1);

    const seenFromGroupOne = async (openThePlane: boolean): Promise<number> => {
      const result = await runtime.runner.run(scheduler, async (uow) => {
        if (openThePlane) {
          await sql`select set_config('app.solver_projection_read', 'solver_projection', true)`.execute(
            uow.query,
          );
        }
        return await sql<{ n: string }>`
          select count(*)::text as n from vacation_selections where id = ${foreignSelectionId}::uuid
        `.execute(uow.query);
      });
    ...
    expect(await seenFromGroupOne(false), 'the control: closed, and invisible').toBe(0);
    expect(
      await seenFromGroupOne(true),
      "the plane widens WHICH ROWS of this group are read, never WHICH GROUP",
    ).toBe(0);
  });
});
